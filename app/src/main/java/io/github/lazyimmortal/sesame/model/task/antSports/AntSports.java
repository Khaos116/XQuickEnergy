...
/**
 * @noinspection NonAsciiCharacters, StatementWithEmptyBody , CommentedOutCode , SameParameterValue , UnusedReturnValue
 */
public class AntSports extends ModelTask {
  ...
  private void signInCoinTask() {
    try {
      ...
      if (data != null && !data.getBoolean("signed")) {
        JSONObject subscribeConfig;
        if (data.has("subscribeConfig")) {
          subscribeConfig = data.optJSONObject("subscribeConfig");
          if (subscribeConfig != null) Log.other("运动任务🧾[做任务得运动币:签到" + subscribeConfig.optString("subscribeExpireDays") + "天]奖励" + data.optString("toast") + "运动币");
        } else {
          //                        Log.record("没有签到");
        }
      } else {
  ...
  private void walk(int syncStepCount) {
    ...
    do {
      ...
      if (isNeedJoinNewPath(goingPathId)) {
        ...
        if (joinPathId != null && checkJoinPathId(joinPathId)) {
  ...
  private Boolean isNeedJoinNewPath(String goingPathId) {
    ...
    try {
      ...
      if (jo == null || jo.optBoolean("dayLimit")) {
        return true;
      }
  ...
  private Boolean hasTreasureBox() {
    ...
    try {
      ...
      if (ja != null) for (int i = 0; i < ja.length(); i++) {
        jo = ja.optJSONObject(i);
        if (!"SPORTSPROD_GOPATH_AWARD_BOX".equals(jo.optString("templateId"))) {
          continue;
        }
  ...
  private Boolean walkGo(JSONObject pathData, int syncStepCount) {
    ...
    try {
      ...
      if (path == null || userPathStep == null) return false;
  ...
  private Boolean walkGo(String pathName, String pathId, int useStepCount) {
    ...
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jo)) {
        ...
        if (jo != null) parseRewardsByJSONObjectData(jo);
      }
    } catch (Throwable t) {
      Log.i(TAG, "walkGo err:");
      Log.printStackTrace(TAG, t);
  ...
  private static void receiveEvent(String eventBillNo) {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jo)) {
        jo = jo.optJSONObject("data");
        if (jo != null) parseRewardsByJSONArrayRewards(jo.optJSONArray("rewards"), 0);
      }
    } catch (Throwable t) {
  ...
  private static void parseRewardsByJSONObjectData(JSONObject data) {
    try {
      ...
      if (data.has("completeInfo")) {
        ...
        if (data != null) {
          JSONArray completeRewards = data.optJSONArray("completeRewards");
  ...
  private String queryGoingPathId() {
    ...
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jo)) {
        jo = jo.optJSONObject("data");
        if (jo != null) goingPathId = jo.optString("goingPathId");
      }
    } catch (Throwable t) {
  ...
  private String queryJoinPathId() {
    ...
    try {
      ...
      if (cityList != null) for (int i = 0; i < cityList.length(); i++) {
        String cityId = cityList.optJSONObject(i).optString("cityId");
        ...
        if (cityPathList != null) for (int j = 0; j < cityPathList.length(); j++) {
  ...
  public static Boolean joinPath(String pathId) {
    ...
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jo)) {
        ...
        JSONObject pt = pathData.optJSONObject("path");
        if (pt == null) return false;
        String pathName = pt.optString("name");
  ...
  private Boolean canDonateCharityCoinToday() {
    ...
    try {
      ...
      jo = jo.optJSONObject("personStatModel");
      if (jo == null) return false;
  ...
  private void queryProjectList() {
    ...
    try {
      ...
      JSONObject pp = jo.optJSONObject("projectPage");
      if (pp == null) return;
      JSONArray ja = pp.optJSONArray("data");
      if (ja == null) return;
      for (int i = 0; i < ja.length(); i++) {
        jo = ja.optJSONObject(i).optJSONObject("basicModel");
        if (jo == null) continue;
  ...
  private void queryWalkStep() {
    ...
    try {
      ...
      //jo = jo.optJSONObject("dailyStepModel");
      ...
      if (walkDonateHomeModel == null) {
        return;
      }
      JSONObject walkUserInfoModel = walkDonateHomeModel.optJSONObject("walkUserInfoModel");
      ...
      if (walkCharityActivityModel == null) {
        return;
      }
      String activityId = walkCharityActivityModel.optString("activityId");
      ...
      JSONObject ua = donateExchangeResultModel.optJSONObject("userAmount");
      if (ua == null) return;
      double amount = ua.getDouble("amount");
      ...
      if (jo.optBoolean("success")) {
        ...
        if (jo.optBoolean("success")) {
          ...
          for (int i = 0; i < dataList.length(); i++) {
            ...
            if (!"P".equals(jo.optString("status"))) {
              continue;
            }
            ...
            for (int j = instanceList.length() - 1; j >= 0; j--) {
              ...
              if (jo.optInt("pointOptions") < pointOptions) {
                continue;
              }
            ...
            if (jo.optBoolean("success")) {
              jo = jo.optJSONObject("data");
              if (jo == null) continue;
              String roundDescription = jo.optString("roundDescription");
              int targetStepCount = jo.optInt("targetStepCount");
  ...
  private void userTaskRightsReceive() {
    try {
      ...
      if (jo.optBoolean("success")) {
        ...
        for (int i = 0; i < userTaskList.length(); i++) {
          jo = userTaskList.optJSONObject(i);
          if (!"COMPLETED".equals(jo.optString("status"))) {
            continue;
          }
          ...
          if (jo.optBoolean("success")) {
            ...
            if (rightsRuleList == null) continue;
      ...
      if (jo.optBoolean("success")) {
        ...
        for (int i = 0; i < userPathGoRewardList.length(); i++) {
          ...
          if (jo.optBoolean("success")) {
            ...
            if (jo == null) continue;
            JSONArray rightsRuleList = jo.optJSONArray("userPathRewardRightsList");
            if (rightsRuleList == null) continue;
            StringBuilder award = new StringBuilder();
            for (int j = 0; j < rightsRuleList.length(); j++) {
              jo = rightsRuleList.optJSONObject(j).optJSONObject("rightsContent");
              if (jo == null) continue;
  ...
  private void queryClubHome() {
    try {
      ...
      if (mainRoom != null && mainRoom.has("bubbleList")) {
        JSONArray bubbleList = mainRoom.optJSONArray("bubbleList");
        if (bubbleList == null) return;
        for (int k = 0; k < bubbleList.length(); k++) {
          String bubbleId = bubbleList.optJSONObject(k).optString("bubbleId");
      ...
      for (int i = 0; i < roomList.length(); i++) {
        ...
        if (room.has("bubbleList")) {
          ...
          if (roombubbleList == null) continue;
          for (int l = 0; l < roombubbleList.length(); l++) {
            String bubbleId = roombubbleList.optJSONObject(l).optString("bubbleId");
            // 收取第i号房间需要收取训练好友的第l个能量球
            collectBubble(bubbleId, "[训练]");
            TimeUtil.sleep(200);
          }
        }
        JSONArray ml = room.optJSONArray("memberList");
      ...
      if (roomListTrain == null) return;
      for (int j = 0; j < roomListTrain.length(); j++) {
        JSONObject roomTrain = roomListTrain.optJSONObject(j);
        JSONArray tml = roomTrain.optJSONArray("memberList");
        if (tml != null && tml.length() != 0) {
          JSONArray tml2 = roomTrain.optJSONArray("memberList");
          if (tml2 == null) continue;
          JSONObject member = tml2.optJSONObject(0);
          trainMember(member);
          TimeUtil.sleep(1000);
        }
      }
      ...
      for (int j = 0; j < roomListTrain.length(); j++) {
        ...
        JSONArray tml3 = roomTrain.optJSONArray("memberList");
        if (tml3 != null && tml3.length() != 0) {
          JSONArray tml4 = roomTrain.optJSONArray("memberList");
          if (tml4 == null) continue;
          JSONObject member = tml4.optJSONObject(0);
          JSONObject trainInfo = member.optJSONObject("trainInfo");
          if (trainInfo == null) continue;
  ...
  private void collectBubble(String bubbleId, String bubbleType) {
    try {
      ...
      if (jo.optBoolean("success")) {
        ...
        if (ja == null) return;
        String collectCoin = ja.optString("changeAmount");
    ...
    try {
      ...
      if (!trainInfo.getBoolean("training")) {
        ...
        if (queryTrainItemjo.has("bizId")) {
          ...
          if (queryTrainItemjo == null) return;
          String taskId = queryTrainItemjo.optString("taskId");
  ...
  private void queryMemberPriceRanking(String roomId) {
    ...
    try {
      ...
      energyBalance = assetsInfo.optInt("energyBalance");
      TimeUtil.sleep(200);
      ...
      for (int i = 0; i < ja.length(); i++) {
        ...
        for (int j = 0; j < roomListTrain.length(); j++) {
          ...
          if (memberList != null && memberList.length() != 0) {
            JSONArray memberList1 = roomTrain.optJSONArray("memberList");
            if (memberList1 == null) continue;
            JSONObject member = memberList1.optJSONObject(0);
            if (originBossId.equals(member.optString("originBossId"))) {
              canbuyMember = false;
            }
          }
        }
  ...
  private JSONObject queryClubMember(JSONObject member) {
    try {
      ...
      if (MessageUtil.checkResultCode(TAG, jo)) {
        JSONObject member1 = jo.optJSONObject("member");
        if (member1 == null) return null;
        JSONObject priceInfo = member1.optJSONObject("priceInfo");
  ...
  public static void receiveSpecialPrize(String sceneType, String rewardName) {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        int energy = data.optInt("modifyCount");
        if (energy > 0) {
  ...
  public static boolean signIn() {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        JSONObject continuousSignInfo = data.optJSONObject("continuousSignInfo");
        if (continuousSignInfo == null) {
          return false;
        }
        int continuousDay = continuousSignInfo.optInt("continuitySignedDayCount");
        JSONObject continuousDoSignInVO = data.optJSONObject("continuousDoSignInVO");
        if (continuousDoSignInVO == null) {
          return false;
        }
        int reward = continuousDoSignInVO.optInt("rewardAmount");
  ...
  public static boolean walkGrid(String branchId, String mapId, String mapName) {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        int step = mapAwards.optJSONObject(0).optInt("step");
        ...
        int currentStar = starData.optInt("curr");
        JSONObject starData1 = data.optJSONObject("starData");
        if (starData1 == null) {
          return false;
        }
        int totalStar = starData1.optInt("count");
    ...
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        for (int i = 0; i < prizes.length(); i++) {
          totalEnergy += prizes.optJSONObject(i).optInt("prizeCount");
        }
  ...
  public static void receiveBubbleReward(String recordId, String rewardName) {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        String energy = data.optString("changeAmount");
        Log.other("悦动健康🚑️领取奖励[" + rewardName + "]#获得[" + energy + "g健康能量]");
      }
    } catch (Exception e) {
      ...
      if (awardspecialActivityReceive.getValue()) {
        ...
        if (data.optBoolean("newGame")) {
          ...
          if (MessageUtil.checkSuccess(TAG, jsonResult)) {
            ...
            if (dataMapDetail == null) {
              return;
            }
            JSONObject baseMapInfo = dataMapDetail.optJSONObject("baseMapInfo");
            if (baseMapInfo == null) {
              return;
            }
            if (baseMapInfo.optInt("currentPercent") == 100 && baseMapInfo.optString("status").equals("FINISH_NOT_REWARD")) {
              ...
              for (int i = 0; i < rewards.length(); i++) {
                ...
                if (reward.optString("prizeStatus").equals("待领取")) {
                  ...
                  if (MessageUtil.checkSuccess(TAG, mapChooseRewardjo)) {
                    ...
                    JSONObject specialActivityReceiveResult = data.optJSONObject("specialActivityReceiveResult");
                    if (specialActivityReceiveResult == null) {
                      return;
                    }
                    JSONArray prizes = specialActivityReceiveResult.optJSONArray("prizes");
                    if (prizes == null) {
                      return;
                    }
                    JSONObject prize = prizes.optJSONObject(0);
  ...
  public static void queryAndProcessBubbleTasks() {
    try {
      ...
      JSONArray tasks = data.optJSONArray("bubbleTaskVOS");
      if (tasks == null) {
        return;
      }
    ...
    try {
      while (hasMore) {
        ...
        for (int i = 0; i < items.length(); i++) {
          ...
          if (!"benefitItem".equals(item.optString("materialType"))) {
            continue;
          }
  ...
  public static boolean canWalkGrid(String branchId, String mapId) {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        return data.getBoolean("canWalk") && starData.optInt("curr") < starData.optInt("count");
      }
    } catch (Exception e) {
  ...
  public static boolean canBuild(String mapId) {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        JSONObject baseMapInfo = data.optJSONObject("baseMapInfo");
        if (baseMapInfo == null) {
          return false;
        }
        return baseMapInfo.getBoolean("newIsLandFlg") && baseMapInfo.optInt("currentPercent") < 100;
      }
    } catch (Exception e) {
  ...
  public static void processTaskCenter() {
    try {
      ...
      JSONArray tasks = data.optJSONArray("taskCenterTaskVOS");
      if (tasks == null) {
        return;
      }
        ...
        if ("SIGNUP_COMPLETE".equals(status)) {
          ...
          if ("LIGHT_TASK".equals(taskType)) {
            if (task.has("logExtMap")) {
              ...
              //if (TaskHelper.checkTaskCompleted(logExtMap.optString("taskType"), logExtMap.optString("bizId"))) {
              //
              //    TimeUtil.sleep(1000);
              //    needRetry = true;
              //}
  ...
  public static int queryUserEnergy() {
    try {
      ...
      if (MessageUtil.checkSuccess(TAG, jsonResult)) {
        ...
        return Integer.parseInt(data.optString("balance"));
      }
    } catch (Exception e) {
  ...
  private void queryMapListSwitch() {
    try {
      ...
      if (MessageUtil.checkSuccess("queryMapList", jsonLandMap)) {
        ...
        JSONArray mapList = data.optJSONArray("mapList");
        if (mapList == null) {
          return;
        }
  ...
  private boolean checkAuth() {
    try {
      ...
      if (MessageUtil.checkSuccess("NeverLandAuth", jsonResult)) {
        ...
        return resultObj.optBoolean("authStatus");
      }
    } catch (Exception e) {
