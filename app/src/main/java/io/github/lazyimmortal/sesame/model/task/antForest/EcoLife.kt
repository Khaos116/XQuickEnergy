...
object EcoLife {
    ...
    fun ecoLife() {
        try {
            ...
            if (AntForestV2.ecoLifeOption?.value?.contains("plate") == true) {
                // 光盘行动
            ...
            // 绿色打卡
            if (AntForestV2.ecoLifeOption?.value?.contains("tick")==true) {
                if (!data.getBoolean("openStatus")) {
                    if (!openEcoLife() || !(AntForestV2.ecoLifeOpen?.value?:false)) {
                        return
                    }
