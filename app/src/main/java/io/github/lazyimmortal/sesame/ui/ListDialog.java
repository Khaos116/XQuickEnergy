...
public class ListDialog {
    ...
    private static void showListDialog(Context c, CharSequence title) {
        if (listDialog == null || listDialog.getContext() != c) {
            listDialog = new AlertDialog.Builder(c).setTitle(title).setView(getListView(c)).setPositiveButton(c.getString(R.string.close), null).create();
        }
    ...
    private static View getListView(Context c) {
        ...
        lv_list.setOnItemClickListener((p1, p2, p3, p4) -> {
            ...
            if (!hasCount) {
                ...
                AlertDialog edtDialog = new AlertDialog.Builder(c).setTitle(curIdAndName.name).setView(edt_count).setPositiveButton(c.getString(R.string.ok), (dialog, which) -> {
                    ...
                    ListAdapter.get(c).notifyDataSetChanged();
                }).setNegativeButton(c.getString(R.string.cancel), null).create();
                if (curIdAndName instanceof CooperateUser) {
                    edt_count.setHint("浇水克数");
                }
                else if (curIdAndName instanceof AlipayMarathon || curIdAndName instanceof AlipayNewAncientTree) {
        ...
        lv_list.setOnItemLongClickListener((p1, p2, p3, p4) -> {
            ...
            if ((curIdAndName instanceof AlipayTree) || (curIdAndName instanceof AlipayReserve) || (curIdAndName instanceof AlipayAnimal) || (curIdAndName instanceof AlipayMarathon) || (curIdAndName instanceof AlipayNewAncientTree) || (curIdAndName instanceof AlipayBeach) || (curIdAndName instanceof AlipayPlantScene) || (curIdAndName instanceof AlipayForestHunt) || (curIdAndName instanceof AlipayMemberCreditSesameTaskList) || (curIdAndName instanceof AlipayAntForestVitalityTaskList) || (curIdAndName instanceof AlipayAntForestHuntTaskList) || (curIdAndName instanceof AlipayAntFarmDoFarmTaskList) || (curIdAndName instanceof AlipayAntFarmDrawMachineTaskList) || (curIdAndName instanceof AlipayAntOceanAntiepTaskList) || (curIdAndName instanceof AlipayAntOrchardTaskList) || (curIdAndName instanceof AlipayAntStallTaskList) || (curIdAndName instanceof AlipayAntSportsTaskList) || (curIdAndName instanceof AlipayAntMemberTaskList) || (curIdAndName instanceof WalkPath)) {
                    ...
                    new AlertDialog.Builder(c).setTitle("删除 " + curIdAndName.name).setPositiveButton(c.getString(R.string.ok), (dialog, which) -> {
                        if (which == DialogInterface.BUTTON_POSITIVE) {
                            if (curIdAndName instanceof AlipayTree) {
                                ...
                                ForestHuntIdMap.save();
                            }
                            else if (curIdAndName instanceof AlipayMemberCreditSesameTaskList) {
                                AlipayMemberCreditSesameTaskList.remove(curIdAndName.id);
                                MemberCreditSesameTaskListMap.remove(curIdAndName.id);
                                MemberCreditSesameTaskListMap.save();
                            }
                            else if (curIdAndName instanceof AlipayAntForestVitalityTaskList) {
                                AlipayAntForestVitalityTaskList.remove(curIdAndName.id);
                                AntForestVitalityTaskListMap.remove(curIdAndName.id);
                                AntForestVitalityTaskListMap.save();
                            }
                            else if (curIdAndName instanceof AlipayAntForestHuntTaskList) {
                                AlipayAntForestHuntTaskList.remove(curIdAndName.id);
                                AntForestHuntTaskListMap.remove(curIdAndName.id);
                                AntForestHuntTaskListMap.save();
                            }
                            else if (curIdAndName instanceof AlipayAntFarmDoFarmTaskList) {
                                AlipayAntFarmDoFarmTaskList.remove(curIdAndName.id);
                                AntFarmDoFarmTaskListMap.remove(curIdAndName.id);
                                AntFarmDoFarmTaskListMap.save();
                            }
                            else if (curIdAndName instanceof AlipayAntFarmDrawMachineTaskList) {
                                AlipayAntFarmDrawMachineTaskList.remove(curIdAndName.id);
                                AntFarmDrawMachineTaskListMap.remove(curIdAndName.id);
                                AntFarmDrawMachineTaskListMap.save();
                            }
                            else if (curIdAndName instanceof AlipayAntOceanAntiepTaskList) {
                                AlipayAntOceanAntiepTaskList.remove(curIdAndName.id);
                                AntOceanAntiepTaskListMap.remove(curIdAndName.id);
                                AntOceanAntiepTaskListMap.save();
                            }
                            else if (curIdAndName instanceof AlipayAntOrchardTaskList) {
                        ...
                        ListAdapter.get(c).notifyDataSetChanged();
                    }).setNegativeButton(c.getString(R.string.cancel), null).create().show();
                }
                catch (Throwable ignored) {
                }
            }
            else if ((curIdAndName instanceof AlipayUser) || (curIdAndName instanceof FriendWatch)) {
                new AlertDialog.Builder(c).setTitle("选项").setAdapter(OptionsAdapter.get(c), new OnClickListener() {
                    Context c;
                ...
                }.setContext(c)).setNegativeButton(c.getString(R.string.cancel), null).create().show();
            }
            return true;
        });
        return v;
